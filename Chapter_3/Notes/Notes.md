...
- `White-space characters:` If `scanf` sees one or more consecutive white-spaces, it will repeatedly ignores until it sees an non-white-space.
- `Other characters:` When it encounters an non-white-space characters, `scanf` compares with the next input character until it doesn't match anymore.

### Adding Fractions with scanf
